...
function mapCmsPage(cms: Record<string, any>, d: typeof alumniPageContent): EplanPageContent {
  ...
  return {
    ...
    sustain: {
      ...
      pillarsEyebrow: txt(cms.sustainPillarsEyebrow, d.sustain.pillarsEyebrow),
      pillars:
    ...
    news: {
      ...
      readMoreLabel: txt(cms.newsReadMoreLabel, d.news.readMoreLabel),
      items: newsItems,
    },
    network: {
...
export async function getEplanPageContent(): Promise<EplanPageContent> {
  ...
  if (sustainFromCms && /^https?:\/\//i.test(sustainFromCms) && !sustainFromCms.includes('/api/media/file/')) {
