function dismissChariticsPreloader() {
...
function chariticsInit() {
    ...
    if (!document.body.dataset.eplReactNav && document.querySelector(".ul-header-nav")) {
        ...
        function updateMenuPosition() {
            if (!ulMobileMenuContent) return;

            if (window.innerWidth < 992 && ulHeaderNavMobileWrapper) {
                ulHeaderNavMobileWrapper.appendChild(ulMobileMenuContent);
            }

            if (window.innerWidth >= 992 && ulHeaderNavOgWrapper) {
    ...
    // ------------------------------- Index 2 js -------------------------------
    // banner slider
    // Hero unified slider — initialized in ChariticsHeroSwiperInit.tsx (client)
...
let chariticsInitAttempts = 0;

function runChariticsInit() {
    // Next.js loads the legacy vendor bundles independently. Wait until the
    // globals used by this template are ready instead of failing when main.js
    // wins the network race (most visibly as "Swiper is not defined").
    const dependenciesReady =
        typeof Swiper !== "undefined" &&
        typeof WOW !== "undefined" &&
        typeof SlimSelect !== "undefined" &&
        typeof Splide !== "undefined";

    if (!dependenciesReady && chariticsInitAttempts < 100) {
        chariticsInitAttempts += 1;
        window.setTimeout(runChariticsInit, 50);
        return;
    }
    ...
    try {
        ...
        console.error("Charitics init error:", error);
    } finally {
        dismissChariticsPreloader();
    }
}
...
if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", runChariticsInit);
} else {
    runChariticsInit();
}
